                ...
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    <form method="POST" action="{{ route('penjual.stockins.store') }}">
                        ...
                        <div class="mt-4">
                            ...
                            <input id="quantity_added" class="block mt-1 w-full rounded-md shadow-sm" type="number" name="quantity_added" value="{{ old('quantity_added', 1) }}" min="1" required />
                            @error('quantity_added') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                        </div>

                        <div class="mt-4">
                            <label for="purchase_price_at_entry" class="block font-medium text-sm text-gray-700 dark:text-gray-300">{{ __('Harga Beli per Unit dari Supplier') }}</label>
                            <input id="purchase_price_at_entry" class="block mt-1 w-full rounded-md shadow-sm" type="number" name="purchase_price_at_entry" value="{{ old('purchase_price_at_entry') }}" min="0" step="50" required />
                            @error('purchase_price_at_entry') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <input id="selling_price_set_at_entry" class="block mt-1 w-full rounded-md shadow-sm" type="number" name="selling_price_set_at_entry" value="{{ old('selling_price_set_at_entry') }}" min="0" step="50" placeholder="Kosongkan jika tidak berubah" />
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Jika diisi, akan mengupdate harga jual varian ini.</p>
                            @error('selling_price_set_at_entry') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                        </div>

                        <div class="mt-4">
                            <label for="entry_date" class="block font-medium text-sm text-gray-700 dark:text-gray-300">{{ __('Tanggal Barang Masuk') }}</label>
                            <input id="entry_date" class="block mt-1 w-full rounded-md shadow-sm" type="date" name="entry_date" value="{{ old('entry_date', now()->format('Y-m-d')) }}" required />
                        ...
                        <div class="mt-4">
                            ...
                            <input id="supplier_name" class="block mt-1 w-full rounded-md shadow-sm" type="text" name="supplier_name" value="{{ old('supplier_name') }}" />
                            @error('supplier_name') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
                        </div>
